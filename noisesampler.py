...
class NoiseSampler:
    ...
    def _get_minutes(self, fname):
        ...
        return len(wavfile.read(wav_path)[1]) / wavfile.read(wav_path)[0] / 60

    def _get_num_frames(self, fname):
        wav_path = self._get_path(fname)
        return len(wavfile.read(wav_path)[1])
    ...
    def _check_snr(self, fname, start_sample, end_sample):
        _, noise = wavfile.read(self._get_path(fname))
        noise = (np.float64(noise) / (2 ** 15))[start_sample:end_sample]
        _, denoise = wavfile.read(self._get_path(fname, read_denoised=True))
        denoise = (np.float64(denoise) / (2 ** 15))[start_sample:end_sample]
        denoise = np.zeros_like(noise) # no iZotope RX 7 Dialogue Isolate tool 
